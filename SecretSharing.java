...
public class SecretSharing {
    ...
    public static void main(String[] a)throws Exception{
        ...
        int[] nk=readNK(j); int n=nk[0],k=nk[1];
        if(n==-1||k==-1){System.err.println("Bad nk");return;}
        Map<String,Map<String,String>> raw=readShares(j);
        List<String> ids=new ArrayList<>(); List<BigInteger> xs=new ArrayList<>(); List<BigInteger> ys=new ArrayList<>();
        ...
        List<int[]> combs=new ArrayList<>(); choose(xs.size(),k,0,0,new int[k],combs);
        ...
        for(int[] c:combs){
            ...
            for(int i=0;i<xs.size();i++){
                ...
                if(!v.same(expected)){ mis++; misIdx.add(i); if(mis>bestMismatch) break; }
            }
            if(mis < bestMismatch){
                bestMismatch = mis;
                bestCoeffs = cf;
